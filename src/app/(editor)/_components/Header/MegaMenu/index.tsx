...
function MenuItem() {
  return (
    <button className="flex items-center space-x-2">
      <span className="text-sm font-medium">Image Filter</span>
      <svg
        width="16"
        height="16"
...
function MegaMenu() {
  ...
  // Get the firstMenu from URL
  // get the secondMenu from URL 
  // its to know what to select if user enters by URL - NExtJS
  ...
  return (
    <div>
      <div className="flex items-center justify-center">
        ...
        <button
          type="button"
          onClick={() => setIsFirstMenuOpen(true)}
        ...
        >
          ...
          <svg width="16" height="16" stroke-linejoin="round" color="var(--ds-gray-900)" data-testid="geist-icon" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M8.707 2.396a1 1 0 0 0-1.414 0L4.47 5.22l-.53.53L5 6.81l.53-.53L8 3.81l2.47 2.47.53.53 1.06-1.06-.53-.53zM5.53 9.72 5 9.19l-1.06 1.06.53.53 2.823 2.824a1 1 0 0 0 1.414 0l2.823-2.824.53-.53L11 9.19l-.53.53L8 12.19z" clip-rule="evenodd"></path></svg>
        </button>
        <svg
          width="16"
          height="16"
        ...
        <button className="flex items-center space-x-2" onClick={() => handleSecondButton()}>
          <span className="text-sm font-medium">Image Filter</span>
          <svg
            width="16"
            height="16"
      ...
      {isFirstMenuOpen &&
        <div className="absolute flex top-40 z-10">
          <div id="topics" className="bg-red-500">
            {topics.map((item) => (
              <div
                key={item.name}
                onMouseEnter={() => handleTopicClick(item)}
          ...
          {isSecondMenuOpen &&
            <div id="chapters" className="bg-green-500">
              {selectedTopic && (
                <div>
                  {topics.find(item => item.id === selectedTopic)?.menu?.map((menuItem) => (
