...
function MegaMenu() {
  ...
  function setInitialNavigationState() {
    const topic = topics.find((topic) => topic.url === segments[0])
    if (topic) {
      const chapter = topic.menu?.find((chapter) => chapter.url === segments[1])
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <div className="flex items-center justify-center">
          ...
          <button className="flex items-center space-x-2 z-[99999] relative" onClick={() => handleChapterButton()}>
        ...
        <div className="flex">
          ...
          {isChapterOpen &&
            <div id="chapters" className="border-l border-l-[#333] my-2 px-2 w-[256px] overflow-y-auto">
              {selectedTopic?.menu && (
                <div className="">
                  {selectedTopic.menu?.map((menuItem) => (
                    <a href={menuItem.url} key={menuItem.name} className="justify-between h-10 text-sm hover:bg-white/10 flex items-center px-2 rounded">
                      <span>{menuItem.name}</span>

                      {menuItem.url === segments[1] &&
                        <span>
                          <svg
                            width="16"
                            height="16"
